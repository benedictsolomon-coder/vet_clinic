...
The priority score is calculated using the formula priorityScore = severityScore - waitScore + apptScore. The severity score assigns a base value of 0 for Critical, 10 for Urgent, 20 for Moderate, and 30 for Routine. A lower total score means higher priority, so the patient at the top of the queue is always the one who needs to be seen most urgently. The system supports up to 100 active patients in the queue at a time and keeps a permanent record of every patient ever registered, including those already served or appointments that had been cancelled.
